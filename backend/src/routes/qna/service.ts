...
const addAnswer = async (questionId: string, userId: string, body: AnswerCreateBody) => {
...
const updateAnswer = async (id: string, userId: string, body: AnswerUpdateBody) => {
  ...
  const row = await prisma.qnaAnswer.update({ where: { id }, data: { body: body.body } });
  return toQnaAnswerItem(row);
};
